...
void rotation_calculation(cv::Mat& giMiniMapRef, double& a, tianli::global::rotation_calculation_config& config);     // Algorithm 1: alpha-channel based, high precision.
void rotation_calculation_2nd(cv::Mat& giMiniMapRef, double& a, tianli::global::rotation_calculation_config& config); // Algorithm 2: screen-space based, low precision.
void rotation_calculation_3nd(cv::Mat& giMiniMapRef, double& a, tianli::global::rotation_calculation_config& config); // Algorithm 3: YUV-space based, low precision.
...
void rotation_calculation(cv::Mat& giMiniMapRef, double& a, tianli::global::rotation_calculation_config& config)
{
    ...
    if (minVal == 255) // No transparent channel.
        return rotation_calculation_2nd(giMiniMapRef, a, config);
    ...
    alpha = alpha(cv::Rect2d(miniMap_size.width * 0.1, 0, miniMap_size.width * 0.3, alpha.size[0]));
    cv::rotate(alpha.clone(), alpha, cv::ROTATE_90_COUNTERCLOCKWISE);
    // Filter.
    cv::Scharr(alpha.clone(), alpha, CV_32FC1, 1, 0);

    // Sum columns and smooth.
    cv::Mat sumOfImg;
...
void rotation_calculation_2nd(cv::Mat& giMiniMapRef, double& a, tianli::global::rotation_calculation_config& config)
{
    ...
    img_object = img_object(cv::Rect2d(miniMap_size.width * 0.1, 0, miniMap_size.width * 0.3, img_object.size[0]));
    ...
    cv::Mat img_edge_arr[2] = { cv::abs(img_object * (1 - alpha) + alpha - cvMatRoll(img_object, cv::Point2d(-5, 0))),
    ...
    {
        config.error = true;
        config.err = { 3006, "Camera rotation error is too large" };
        a = lastRotate;
        return;
    }
