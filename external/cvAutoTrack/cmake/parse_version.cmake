...
function(auto_increment_version_file old_hash_file version_file)
    if(NOT IS_ABSOLUTE "${old_hash_file}")
        set(old_hash_file "${CMAKE_SOURCE_DIR}/${old_hash_file}")
    endif()
    if(NOT IS_ABSOLUTE "${version_file}")
        set(version_file "${CMAKE_SOURCE_DIR}/${version_file}")
    endif()
    get_commit_hash(new_hash)
    message(STATUS "Current version hash: ${new_hash}")
    ...
    if(EXISTS "${old_hash_file}")
        ...
        message(STATUS "Previous build version hash: ${old_hash}")
        if(NOT "${old_hash}" STREQUAL "${new_hash}")
            message(STATUS "Version hash changed; updating version number automatically.")
            version_file_patch_update("${version_file}")
            file(WRITE "${old_hash_file}" "${new_hash}")
        endif()
    else()
        message(STATUS "Version hash file does not exist; updating version number automatically.")
        version_file_patch_update("${version_file}")
        file(WRITE "${old_hash_file}" "${new_hash}")
    endif()
